  ...
  public class UIHandler : MonoBehaviour {
          ...
          if (e2.Snapshot != null && e2.Snapshot.ChildrenCount > 0) {
            foreach (var childSnapshot in e2.Snapshot.Children) {
              if (childSnapshot.Child("score") == null
				...
				childSnapshot.Child("uuid").Value.ToString() + " - " +
    ...
    TransactionResult AddScoreTransaction(MutableData mutableData) {
	  ...
	  if (leaders == null) {
		...
		newScoreMap["uuid"] = SystemInfo.deviceUniqueIdentifier;
		newScoreMap["score"] = score;
		newScoreMap["username"] = username;
		leaders.Add(newScoreMap);
      } else {
         for(int i=0; i < leaders.Count; i++) {
			if(((Dictionary<string, object>)leaders[i])["uuid"].Equals(SystemInfo.deviceUniqueIdentifier)) {
				Debug.Log("Inside");
				if(int.Parse(((Dictionary<string, object>)leaders[i])["score"].ToString()) < score){
					((Dictionary<string, object>)leaders[i])["uuid"] = SystemInfo.deviceUniqueIdentifier;
					((Dictionary<string, object>)leaders[i])["score"] = score;
					((Dictionary<string, object>)leaders[i])["username"] = username;
					break;
				}
				else
					addData = false;
    ...
    public void AddScore() {
      if (score == 0 || string.IsNullOrEmpty(username)) {
        DebugLog("invalid score or username.");
        return;
      }
    ...
    void GUIDisplayControls() {
      if (UIEnabled) {
        ...
        GUILayout.Label("username:", GUILayout.Width(Screen.width * 0.20f));
        ...
        if (!String.IsNullOrEmpty(username) && GUILayout.Button("Enter Score")) {
          AddScore();
        }
